...
```{r, echo=FALSE} 
pkgs = c("alr4","rmarkdown", "faraway", "datasets", "sos", "car", "plotly", "ggplot2", "tidyverse", "broom", "SemiPar", "RegularExpressions","GGally", "leaps")
...
#Take out insignificant variable to satisfy normality assumption
...
lm9 = lm(data = na.omit(cov19_project_nm), logcasesmonth ~ r)




step(lm8reduced, scope = list(lower = lm8reduced, upper = lm8))
...
```{r}
library(leaps)
mod = regsubsets(cbind(na.omit(cov19_project_nm)$log_percent_low_birthweight, na.omit(cov19_project_nm)$percent_smokers, na.omit(cov19_project_nm)$log_primary_care_physicians_rate, na.omit(cov19_project_nm)$householdspercapita, na.omit(cov19_project_nm)$average_daily_pm2_5, na.omit(cov19_project_nm)$life_expectancy, na.omit(cov19_project_nm)$log_median_household_income, na.omit(cov19_project_nm)$log_percent_65_and_over, na.omit(cov19_project_nm)$majorityruralfactor), na.omit(cov19_project_nm)$logcasesmonth)
...
#Take out variables that seem less relevant
