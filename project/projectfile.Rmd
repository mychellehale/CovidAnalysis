...
varformap = SWTWCdata$`PUBLIC ROAD MILES PER 1,000 PEOPLE`

SWTWCdataframe = data.frame(varformap, state)
#SWTWCdataframe
plot_usmap(regions = c("states"), data = cov19_project_nm,
           values = "state", color = "black", labels = FALSE,
           label_color = "black") + scale_fill_continuous(low = "light blue", high = "blue", na.value = "light gray", name ="", label = scales::comma) +
...
lm8mobility = lm(data = na.omit(cov19_project), logcasesmonth ~  log_percent_low_birthweight + percent_smokers +log_primary_care_physicians_rate + average_daily_pm2_5 + life_expectancy + log_infant_mortality_rate +log_median_household_income + log_percent_65_and_over + majorityruralfactor + retail_recreation + grocery_pharmacy +parks + transit_stations + workplaces + residential)


anova(lm8, lm8mobility)
...
fit1 = lm(logcasesmonth ~ log_percent_65_and_over, data = na.omit(cov19_project_nm))
fit2 = lm(logcasesmonth ~ log_percent_low_birthweight + percent_smokers +log_primary_care_physicians_rate +average_daily_pm2_5 +life_expectancy +log_infant_mortality_rate +log_median_household_income + log_percent_65_and_over +majorityruralfactor + log_median_household_income*majorityruralfactor, data = na.omit(cov19_project_nm))
summary(fit1)
summary(fit2)
